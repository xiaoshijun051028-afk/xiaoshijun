...
## 尺寸放大（用户试玩 2026-08-12：特效要更大）。
func slash(pos: Vector3) -> void:
	var ring := TorusMesh.new()
	ring.inner_radius = 0.75
	ring.outer_radius = 1.7
	_fx(ring, ColorTokens.RESONANCE_GLOW, pos, Vector3(-90.0, 0.0, 0.0), 0.5, 2.9, 0.38)
...
func dash_trail(pos: Vector3) -> void:
	...
	_fx(streak, ColorTokens.PLAYER_ALLY_MAIN, pos, Vector3.ZERO, 0.9, 0.07, 0.32)
...
func perfect_parry(pos: Vector3) -> void:
	...
	_fx(ring, ColorTokens.FRIENDLY_GOLD, pos, Vector3(-90.0, 0.0, 0.0), 0.6, 3.4, 0.48)
...
func finisher(pos: Vector3) -> void:
	...
	ring.outer_radius = 1.6
	_fx(ring, ColorTokens.RESONANCE_GLOW, pos, Vector3(-90.0, 0.0, 0.0), 0.8, 4.4, 0.62)
	var core := SphereMesh.new()
	core.radius = 0.62
	core.height = 1.24
	_fx(core, ColorTokens.RESONANCE_GLOW, pos, Vector3.ZERO, 0.5, 2.5, 0.62)


## 角色技能发动（统一青白谐波虹膜，与斩击/终结技同一套视觉语言，不引入第二种色）。
func skill_cast(pos: Vector3) -> void:
	finisher(pos)
	var ring := TorusMesh.new()
	ring.inner_radius = 0.5
	ring.outer_radius = 1.5
	_fx(ring, ColorTokens.RESONANCE_GLOW, pos, Vector3(-90.0, 0.0, 0.0), 0.6, 3.8, 0.52)
